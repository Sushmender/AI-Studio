...
async def generate_video(prompt: str, aspect_ratio: str = "16:9", duration: int = 5, job_id: str = "", image_url: Optional[str] = None) -> dict:
    settings = get_settings()
    
    if settings.mock_generation_apis:
        logger.info("mock_replicate_generation", prompt_preview=prompt[:80], has_reference_image=bool(image_url))
        await asyncio.sleep(4)  # Simulate some processing time
        return {"url": "https://replicate.delivery/pbxt/mock_video.mp4"}

    try:
        # Build input dict — conditionally include image_url for image-to-video
        input_params = {
            "prompt": prompt,
        ...
        if image_url:
            ...
            logger.info("replicate_image_to_video", image_url=image_url[:80])
